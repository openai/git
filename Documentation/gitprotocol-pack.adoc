...
unrecognized keys. The "version" Extra Parameter takes a value of '1' or
'2'. See linkgit:gitprotocol-v2[5] for more information on protocol
version 2.

For smart HTTP receive-pack discovery, a client that supports the
'explicit-haves' capability can send the bare 'explicit-haves' Extra
Parameter before the server advertises refs. See linkgit:gitprotocol-capabilities[5]
for how this parameter relates to the receive-pack capability of the same name.
...
'ofs-delta', 'atomic', 'push-options' and 'explicit-haves'.
...
When 'explicit-haves' is in effect, the client MUST NOT infer pack
prerequisites from the object IDs of named refs in the reference
advertisement. Only ".have" pseudo-refs and common objects explicitly
acknowledged by push negotiation may be used as pack prerequisites. The
advertised object ID of a named ref is still used as the old-id in the
