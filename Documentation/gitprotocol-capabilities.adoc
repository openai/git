...
'explicit-haves', 'no-ref-delta', 'quiet', and 'push-cert' capabilities
...
The receive-pack server can advertise 'explicit-haves' to indicate that
the object IDs of its named refs are not necessarily pack prerequisites.
When a client requests this capability, it MUST NOT treat an object as a
pack prerequisite merely because the server advertised a named ref that
points to it.

Objects named by ".have" pseudo-refs and common objects explicitly
acknowledged by push negotiation remain valid pack prerequisites. This
capability does not otherwise change how the client uses advertised refs,
including for reference matching and update preconditions.

A client that supports 'explicit-haves' SHOULD request it whenever the
server advertises it. A server that relies on this behavior MUST verify
that the client requested the capability before accepting the pack.

For smart HTTP, such a client SHOULD also send the bare 'explicit-haves'
Extra Parameter in its initial receive-pack discovery request, before
the server advertises refs. This lets a server choose which refs it can
advertise to that client. The Extra Parameter does not activate the
capability: the server must still advertise it, and the client must
still request it in the update command. A server that ignores the Extra
Parameter and does not advertise the capability retains the usual pack
